...
let i = message.guild.shardID
    ...
    for(i=0;i<client.shard.count;i++) {
        spectrum.addField(`Shard: ${i+1} | Ping: ${Math.round(shardinfo.ping[i])}ms `, ` ${shardinfo.server_count[i]} sunucu ve ${shardinfo.user_count[i]} kullanıcı\nUptime: ${moment.duration(shardinfo.uptime[i]).format(`D [Gün] , H [Saat], m [Dakika], s [Saniye]`)} `)
    }
    ...
    const spectrum = new Discord.MessageEmbed()
      .setDescription(
        `Sanırım bir sorun var! Bunu yetkililere bildir!\nDestek Sunucusu: [Tıkla!](https://discord.gg/rP7TNATQnZ) \nHata kodu: ${err}`
      )
      .setColor("RED")
      .setThumbnail(client.user.avatarURL())
      .setTimestamp();
